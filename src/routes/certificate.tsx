...
import { useProgress, isCertificateReady } from "@/store/progress";
...
function CertificatePage() {
  ...
  const mast = mastered.length;
  const pct = LESSONS.length ? Math.round((mast / LESSONS.length) * 100) : 0;
  ...
  return (
    <div className="mx-auto max-w-lg pb-16">
      <header className="mb-6 text-center">
        ...
        <p className="mt-1 text-sm text-muted">全部课程测验 ≥80% 掌握后解锁</p>
      </header>

      <section className="mb-4 rounded-xl border border-border bg-surface p-4">
        <h2 className="text-sm font-medium text-fg">各路径掌握度</h2>
        <ul className="mt-3 space-y-2">
          {orderedTracks().map((t) => {
            const list = getLessonsByTrack(t);
            const m = list.filter((l) => mastered.includes(l.slug)).length;
      ...
      {!unlocked ? (
        <section className="rounded-xl border border-border bg-surface p-8 text-center">
          ...
          <p className="mt-4 text-sm text-muted">
            掌握 {mast}/{LESSONS.length}（测验≥80%）· 完成 {completed.length}/
            {LESSONS.length}
          </p>
        ...
        <section className="rounded-xl border border-primary/30 bg-surface p-8 text-center shadow-soft">
          ...
          <h2 className="mt-4 font-display text-xl font-semibold text-fg">D3 结业</h2>
          <p className="mt-2 text-sm text-muted">
            兹证明持有人已掌握本站路径（测验 ≥80%），并完成工坊 / 演练场等实践模块。
          </p>
          <dl className="mx-auto mt-6 grid max-w-xs grid-cols-2 gap-3 text-left text-sm">
            <div className="rounded-lg bg-surface-2 px-3 py-2">
              <dt className="text-xs text-subtle">掌握</dt>
            ...
            <div className="rounded-lg bg-surface-2 px-3 py-2">
              ...
              <dd className="font-mono text-primary">{completed.length}</dd>
            </div>
          </dl>
        </section>
      )}
